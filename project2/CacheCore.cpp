...
*CacheAssoc<State, Addr_t, Energy>::findLine2Replace(Addr_t addr, bool ignoreLocked)
{
    ...
    // Start in normal order so that get the least recently used invalid possible,
    // and the youngest isLocked possible (lineFree)
    {
        Line **l = setEnd -1;
	Line **lMRU = theSet;
	...
	if (policy == MRU) {
		while(lMRU <= setEnd - 1) {
            ...
            if (!(*lMRU)->isValid())
                lineFree = lMRU;
            else if (lineFree == 0 && !(*lMRU)->isLocked())
                lineFree = lMRU;

            // If line is invalid, isLocked must be false
            GI(!(*lMRU)->isValid(), !(*lMRU)->isLocked());
            lMRU++;
        }
    ...
    if (lineFree == 0) {
	    ...
	    I(policy == MRU);  // doassert() if this condition is false
	    lineFree = theSet; // for MRU - the entry in the first position of the set	
        ...
        if (policy == RANDOM && (*lineFree)->isValid()) {
            lineFree = &theSet[irand];
            irand = (irand + 1) & maskAssoc;
        } else if (policy == MRU && (*lineFree)->isValid()) {
	   // I(policy == MRU);
	   // Do nothing. lineFree is the most recently used entry
	   // lineFree = lineFreeMRU; 
	} else {
            //      I(policy == LRU);
